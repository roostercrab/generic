defmodule Generic.Accounts do
  ...
  alias Generic.Accounts.{Users, UsersToken, UsersNotifier}
  ...
  def update_users_email(users, token) do
    ...
    with {:ok, query} <- UsersToken.verify_users_change_email_token_query(token, context),
  ...
  def deliver_update_email_instructions(%Users{} = users, current_email, update_email_url_fun)
    ...
    {encoded_token, users_token} =
      UsersToken.build_users_email_token(users, "change:#{current_email}")
  ...
  """
  def generate_session_token(users) do
  ...
  def deliver_users_confirmation_instructions(%Users{} = users, confirmation_url_fun)
    ...
    if users.confirmed_at do
      ...
      {encoded_token, users_token} = UsersToken.build_users_email_token(users, "confirm")
      Repo.insert!(users_token)
  ...
  def confirm_users(token) do
    with {:ok, query} <- UsersToken.verify_users_email_token_query(token, "confirm"),
  ...
  def get_users_by_reset_password_token(token) do
    with {:ok, query} <- UsersToken.verify_users_email_token_query(token, "reset_password"),
